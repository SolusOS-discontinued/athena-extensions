...
/*
  ...
  finish_file info command is in athena_dropbox,
  this can be cleaned up once the file_info_command isn't a special
  case anylonger
*/
gboolean athena_dropbox_finish_file_info_command(DropboxFileInfoCommandResponse *);
...
do_file_info_command(GIOChannel *chan, DropboxFileInfoCommand *dfic, GError **gerr) {
...

  {
    ...
    uri = athena_file_info_get_uri(dfic->file);
  ...
  if (athena_file_info_is_directory(dfic->file)) {
    args = g_hash_table_new_full((GHashFunc) g_str_hash,
				 (GEqualFunc) g_str_equal,
				 (GDestroyNotify) g_free,
				 (GDestroyNotify) g_strfreev);
...
exit:
  ...
  g_idle_add((GSourceFunc) athena_dropbox_finish_file_info_command, dficr);
...
void dropbox_command_client_send_command(DropboxCommandClient *dcc, 
					 AthenaDropboxCommandResponseHandler h,
					 gpointer ud,
					 const char *command, ...) {
  va_list ap;
  DropboxGeneralCommand *dgc;
