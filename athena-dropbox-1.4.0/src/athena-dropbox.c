 ...
 *
 * athena-dropbox.c
 * Implements the Athena extension API for Dropbox. 
 *
...
gboolean dropbox_use_athena_submenu_workaround;
...
static void
reset_file(AthenaFileInfo *file) {
  debug("resetting file %p", (void *) file);
  athena_file_info_invalidate_extension_info(file);
}

gboolean
reset_all_files(AthenaDropbox *cvs) {
...
static void
when_file_dies(AthenaDropbox *cvs, AthenaFileInfo *address) {
  gchar *filename;
...
static void
changed_cb(AthenaFileInfo *file, AthenaDropbox *cvs) {
  ...
  /* this is a hack, because athena doesn't do this for us, for some reason
...
athena_dropbox_update_file_info(AthenaInfoProvider     *provider,
                                  AthenaFileInfo         *file,
                                  GClosure                 *update_complete,
                                  AthenaOperationHandle **handle) {
  AthenaDropbox *cvs;

  cvs = ATHENA_DROPBOX(provider);
     ...
     so we can shell touch these files later */
  {
    ...
    g_free(uri);
    if (pfilename == NULL) {
      return ATHENA_OPERATION_COMPLETE;
    }
    else {
      int cmp = 0;
	...
	if (stored_filename != NULL && cmp != 0) {
	  ...
	  if ((f2 = g_hash_table_lookup(cvs->filename2obj, filename)) != NULL) {
	    /* if the filename exists in the filename2obj hash
	       ...
	       this happens when athena allocates another file object
...

  {
    ...
    *handle = (AthenaOperationHandle *) dfic;
...
static void
handle_shell_touch(GHashTable *args, AthenaDropbox *cvs) {
  gchar **path;
...
gboolean
athena_dropbox_finish_file_info_command(DropboxFileInfoCommandResponse *dficr) {

  //debug_enter();
  AthenaOperationResult result = ATHENA_OPERATION_FAILED;

  if (!dficr->dfic->cancelled) {
    gchar **status = NULL;
    gboolean isdir;

    isdir = athena_file_info_is_directory(dficr->dfic->file) ;
	...
	(status = g_hash_table_lookup(dficr->emblems_response, "emblems")) != NULL) {
	  ...
	  if (status[i][0])
	    athena_file_info_add_emblem(dficr->dfic->file, status[i]);
      }
      result = ATHENA_OPERATION_COMPLETE;
    }
	...
	if (strcmp("public", tag[0]) == 0) {
	  athena_file_info_add_emblem(dficr->dfic->file, "web");
	}
	else if (strcmp("shared", tag[0]) == 0) {
	  athena_file_info_add_emblem(dficr->dfic->file, "people");
	}
	else if (strcmp("photos", tag[0]) == 0) {
	  athena_file_info_add_emblem(dficr->dfic->file, "photos");
	}
	else if (strcmp("sandbox", tag[0]) == 0) {
	  athena_file_info_add_emblem(dficr->dfic->file, "star");
	}
      }
	...
	if (emblem_code > 0) {
	  /*
	    ...
	    g_filename_from_uri(athena_file_info_get_uri(dficr->dfic->file),
	    NULL, NULL));
	  */
	  athena_file_info_add_emblem(dficr->dfic->file, emblems[emblem_code-1]);
	}
      }
      result = ATHENA_OPERATION_COMPLETE;
    }
  ...
  if (!dropbox_use_operation_in_progress_workaround) {
      athena_info_provider_update_complete_invoke(dficr->dfic->update_complete,
						    dficr->dfic->provider,
						    (AthenaOperationHandle*) dficr->dfic,
						    result);
  }
...
static void
athena_dropbox_cancel_update(AthenaInfoProvider     *provider,
                               AthenaOperationHandle  *handle) {
...
static void
menu_item_cb(AthenaMenuItem *item,
	     AthenaDropbox *cvs) {
  gchar *verb;
  GList *files;
  ...
  files = g_object_get_data(G_OBJECT(item), "athena_dropbox_files");
  verb = g_object_get_data(G_OBJECT(item), "athena_dropbox_verb");
					     ...
					     (GDestroyNotify) g_strfreev);
  {
    ...
    for (li = files, i = 0; li != NULL; li = g_list_next(li)) {
      char *uri = athena_file_info_get_uri(ATHENA_FILE_INFO(li->data));
...
static int
athena_dropbox_parse_menu(gchar			**options,
			    AthenaMenu		*menu,
			    ...
			    GList			*files)
{
  ...
  for ( i = 0; options[i] != NULL; i++) {
    ...
    if (strchr(item_inner, '~') != NULL) {
      ...
      AthenaMenuItem *item;
      AthenaMenu *submenu = athena_menu_new();

      g_string_append(new_action_string, item_name);
      g_string_append(new_action_string, "::");

      ret += athena_dropbox_parse_menu(suboptions, submenu, new_action_string,
					 toret, provider, files);

      item = athena_menu_item_new(new_action_string->str,
				    item_name, "", NULL);
      athena_menu_item_set_submenu(item, submenu);
      athena_menu_append_item(menu, item);
      ...
      item = athena_menu_item_new(new_action_string->str, item_name, item_inner, NULL);

      athena_menu_append_item(menu, item);
      /* add the file metadata to this item */
      g_object_set_data_full (G_OBJECT(item), "athena_dropbox_files",
			      athena_file_info_list_copy (files),
			      (GDestroyNotify) athena_file_info_list_free);
      /* add the verb metadata */
      g_object_set_data_full (G_OBJECT(item), "athena_dropbox_verb",
			      g_strdup(verb),
			      (GDestroyNotify) g_free);
	...
	g_object_set_property (G_OBJECT(item), "sensitive", &sensitive);
      ...
      /* taken from athena-file-repairer (http://repairer.kldp.net/):
       * this code is a workaround for a bug of athena
       * See: http://bugzilla.gnome.org/show_bug.cgi?id=508878 */
      if (dropbox_use_athena_submenu_workaround) {
...
static GList *
athena_dropbox_get_file_items(AthenaMenuProvider *provider,
                                GtkWidget            *window,
				GList                *files)
{
  /*
  ...
  for (elem = files; elem; elem = elem->next, i++) {
    gchar *uri = athena_file_info_get_uri(elem->data);
   ...
   */
  AthenaDropbox *cvs = ATHENA_DROPBOX(provider);
  dropbox_command_client_request(&(cvs->dc.dcc), (DropboxCommand *) dgc);

  GTimeVal gtv;

  /*
   * 4. We have to block until it's done because athena expects a reply.  But we will
  ...
  if (options && *options && **options)  {
    /* build the menu */
    AthenaMenuItem *root_item;
    AthenaMenu *root_menu;

    root_menu = athena_menu_new();
    root_item = athena_menu_item_new("AthenaDropbox::root_item",
				       "Dropbox", "Dropbox Options", "dropbox");

    toret = g_list_append(toret, root_item);
    GString *action_string = g_string_new("AthenaDropbox::");

    if (!athena_dropbox_parse_menu(options, root_menu, action_string,
				     toret, provider, files)) {
	g_object_unref(toret);
	toret = NULL;
    }

    athena_menu_item_set_submenu(root_item, root_menu);
...
void get_emblem_paths_cb(GHashTable *emblem_paths_response, AthenaDropbox *cvs)
{
  if (!emblem_paths_response) {
...
static void
on_connect(AthenaDropbox *cvs) {
  reset_all_files(cvs);

  dropbox_command_client_send_command(&(cvs->dc.dcc),
				      (AthenaDropboxCommandResponseHandler) get_emblem_paths_cb,
				      cvs, "get_emblem_paths", NULL);
}

static void
on_disconnect(AthenaDropbox *cvs) {
  reset_all_files(cvs);
...
static void
athena_dropbox_menu_provider_iface_init (AthenaMenuProviderIface *iface) {
  iface->get_file_items = athena_dropbox_get_file_items;
  return;
}

static void
athena_dropbox_info_provider_iface_init (AthenaInfoProviderIface *iface) {
  iface->update_file_info = athena_dropbox_update_file_info;
  iface->cancel_update = athena_dropbox_cancel_update;
  return;
}

static void
athena_dropbox_instance_init (AthenaDropbox *cvs) {
  ...
  /* our hooks */
  athena_dropbox_hooks_add(&(cvs->dc.hookserv), "shell_touch",
...
static void
athena_dropbox_class_init (AthenaDropboxClass *class) {
}

static void
athena_dropbox_class_finalize (AthenaDropboxClass *class) {
  debug("just checking");
  /* kill threads here? */
}

GType
athena_dropbox_get_type (void) {
  return dropbox_type;
}

void
athena_dropbox_register_type (GTypeModule *module) {
  static const GTypeInfo info = {
    sizeof (AthenaDropboxClass),
    (GBaseInitFunc) NULL,
    (GBaseFinalizeFunc) NULL,
    (GClassInitFunc) athena_dropbox_class_init,
    (GClassFinalizeFunc) athena_dropbox_class_finalize,
    NULL,
    sizeof (AthenaDropbox),
    0,
    (GInstanceInitFunc) athena_dropbox_instance_init,
  };

  static const GInterfaceInfo menu_provider_iface_info = {
    (GInterfaceInitFunc) athena_dropbox_menu_provider_iface_init,
    NULL,
    NULL
  };

  static const GInterfaceInfo info_provider_iface_info = {
    (GInterfaceInitFunc) athena_dropbox_info_provider_iface_init,
    NULL,
    NULL
  };
