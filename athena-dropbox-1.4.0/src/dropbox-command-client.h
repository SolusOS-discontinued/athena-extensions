 ...
 * Header file for athena-dropbox-command.c
 *
...
/* command structs */
typedef enum {GET_FILE_INFO, GENERAL_COMMAND} AthenaDropboxRequestType;

typedef struct {
  AthenaDropboxRequestType request_type;
} DropboxCommand;
...
typedef void (*AthenaDropboxCommandResponseHandler)(GHashTable *, gpointer);

typedef struct {
  DropboxCommand dc;
  gchar *command_name;
  GHashTable *command_args;
  AthenaDropboxCommandResponseHandler handler;
  gpointer handler_ud;
