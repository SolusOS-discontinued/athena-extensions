...
command_on_disconnect(DropboxClient *dc) {
  ...
  if (dc->hook_disconnect_called) {
    ...
    athena_dropbox_hooks_force_reconnect(&(dc->hookserv));
  }
}
...
dropbox_client_is_connected(DropboxClient *dc) {
  return (dropbox_command_client_is_connected(&(dc->dcc)) &&
	  athena_dropbox_hooks_is_connected(&(dc->hookserv)));
}
...
dropbox_client_setup(DropboxClient *dc) {
  athena_dropbox_hooks_setup(&(dc->hookserv));
  ...
  athena_dropbox_hooks_add_on_connect_hook(&(dc->hookserv), 
  ...
  athena_dropbox_hooks_add_on_disconnect_hook(&(dc->hookserv), 
						(DropboxHookClientConnectHook)
...
dropbox_client_start(DropboxClient *dc) {
  ...
  athena_dropbox_hooks_start(&(dc->hookserv));
