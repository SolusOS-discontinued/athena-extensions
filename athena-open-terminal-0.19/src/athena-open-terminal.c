...
static void athena_open_terminal_instance_init (AthenaOpenTerminal      *cvs);
static void athena_open_terminal_class_init    (AthenaOpenTerminalClass *class);
...
get_terminal_file_info (const char *uri)
{
	...
	if (uri_scheme == NULL) {
		...
		ret = FILE_INFO_LOCAL;
	} else if (strcmp (uri_scheme, "x-athena-desktop") == 0) {
...
desktop_opens_home_dir (void)
{
	return gconf_client_get_bool (gconf_client,
				      "/apps/athena-open-terminal/desktop_opens_home_dir",
				      NULL);
}
...
{
	return gconf_client_get_bool (gconf_client,
				      "/apps/athena-open-terminal/display_mc_item",
				      NULL);
}
...
{
	return gconf_client_get_bool (gconf_client,
				      "/apps/athena/preferences/desktop_is_home_dir",
				      NULL);
}
...
static char *
get_terminal_command_for_file_info (AthenaFileInfo *file_info,
				    const char *command_to_run,
				    gboolean remote_terminal)
{
	...
	uri = athena_file_info_get_activation_uri (file_info);
...
static void
open_terminal (AthenaMenuItem *item,
	       AthenaFileInfo *file_info)
{
	char *terminal_command, *command_to_run;
	GdkScreen *screen;
	gboolean remote_terminal;

	screen = g_object_get_data (G_OBJECT (item), "AthenaOpenTerminal::screen");
	command_to_run = g_object_get_data (G_OBJECT (item), "AthenaOpenTerminal::command-to-run");
	remote_terminal = GPOINTER_TO_UINT (g_object_get_data (G_OBJECT (item), "AthenaOpenTerminal::remote-terminal"));
...
static void
open_terminal_callback (AthenaMenuItem *item,
			AthenaFileInfo *file_info)
{
	open_terminal (item, file_info);
}

static AthenaMenuItem *
open_terminal_menu_item_new (AthenaFileInfo *file_info,
			     ...
			     gboolean          is_file_item)
{
	...
	if (command_to_run != NULL) {
		action_name = g_strdup_printf (remote_terminal ?
					       "AthenaOpenTerminal::open_remote_terminal_%s" :
					       "AthenaOpenTerminal::open_terminal_%s",
					       command_to_run);
	} else {
		action_name = g_strdup (remote_terminal ? 
					"AthenaOpenTerminal::open_remote_terminal" :
					"AthenaOpenTerminal::open_terminal");
	}
	ret = athena_menu_item_new (action_name, name, tooltip, "gnome-terminal");
	g_free (action_name);

	g_object_set_data (G_OBJECT (ret),
			   "AthenaOpenTerminal::screen",
			   screen);
	g_object_set_data_full (G_OBJECT (ret), "AthenaOpenTerminal::command-to-run",
				g_strdup (command_to_run),
				(GDestroyNotify) g_free);
	g_object_set_data (G_OBJECT (ret), "AthenaOpenTerminal::remote-terminal",
...
static GList *
athena_open_terminal_get_background_items (AthenaMenuProvider *provider,
					     GtkWidget		  *window,
					     AthenaFileInfo	  *file_info)
{
	gchar *uri;
...
GList *
athena_open_terminal_get_file_items (AthenaMenuProvider *provider,
				       GtkWidget            *window,
				       GList                *files)
{
	gchar *uri;
	...
	if (g_list_length (files) != 1 ||
	    (!athena_file_info_is_directory (files->data) &&
	     athena_file_info_get_file_type (files->data) != G_FILE_TYPE_SHORTCUT &&
	     athena_file_info_get_file_type (files->data) != G_FILE_TYPE_MOUNTABLE)) {
		return NULL;
	}

	items = NULL;

	uri = athena_file_info_get_activation_uri (files->data);
	terminal_file_info = get_terminal_file_info (uri);
...
static void
athena_open_terminal_menu_provider_iface_init (AthenaMenuProviderIface *iface)
{
	iface->get_background_items = athena_open_terminal_get_background_items;
	iface->get_file_items = athena_open_terminal_get_file_items;
}

static void 
athena_open_terminal_instance_init (AthenaOpenTerminal *cvs)
{
}

static void
athena_open_terminal_class_init (AthenaOpenTerminalClass *class)
{
...
static void
athena_open_terminal_class_finalize (AthenaOpenTerminalClass *class)
{
	g_assert (gconf_client != NULL);
...
GType
athena_open_terminal_get_type (void) 
{
	return terminal_type;
}

void
athena_open_terminal_register_type (GTypeModule *module)
{
	static const GTypeInfo info = {
		sizeof (AthenaOpenTerminalClass),
		(GBaseInitFunc) NULL,
		(GBaseFinalizeFunc) NULL,
		(GClassInitFunc) athena_open_terminal_class_init,
		(GClassFinalizeFunc) athena_open_terminal_class_finalize,
		NULL,
		sizeof (AthenaOpenTerminal),
		0,
		(GInstanceInitFunc) athena_open_terminal_instance_init,
	};

	static const GInterfaceInfo menu_provider_iface_info = {
		(GInterfaceInitFunc) athena_open_terminal_menu_provider_iface_init,
		NULL,
		NULL
	};
