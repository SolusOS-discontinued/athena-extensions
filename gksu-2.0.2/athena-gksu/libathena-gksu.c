...
static void
menu_provider_iface_init (AthenaMenuProviderIface *iface);

static GList*
gksu_context_menu_get_file_items (AthenaMenuProvider *provider,
				  GtkWidget *window,
				  GList *files);
static void
gksu_context_menu_activate (AthenaMenuItem *item,
			    AthenaFileInfo *file);
...
static void menu_provider_iface_init (AthenaMenuProviderIface *iface)
{
				  ...
				  GList *files)
{
    ...
    /* ... and if it is not a athena special item */
    {
      gchar *uri_scheme = NULL;

      uri_scheme = athena_file_info_get_uri_scheme (file);
      if (!strncmp (uri_scheme, "x-athena-desktop", 18))
	{
	  g_free (uri_scheme);
	  return NULL;
	}
    ...
    item = athena_menu_item_new ("Gksu::open_as_root",
