...
fr_window_free_private_data (FrWindow *window)
{
	...
	_g_object_unref (window->priv->settings_athena);
}
...
static gboolean
athena_xds_dnd_is_valid_xds_context (GdkDragContext *context)
{
	char *tmp;
	gboolean ret;
				     ...
				     gpointer          user_data)
{
	...
	if (! athena_xds_dnd_is_valid_xds_context (context))
		return FALSE;
...
is_single_click_policy (FrWindow *window)
{
	...
	if (window->priv->settings_athena == NULL)
		return FALSE;

	value = g_settings_get_string (window->priv->settings_athena, ATHENA_CLICK_POLICY);
...
fr_window_construct (FrWindow *window)
{
	...
	/* Only use the athena schema if it's installed */
	for (schemas = g_settings_list_schemas ();
	     *schemas != NULL;
	     schemas++)
	{
		if (g_strcmp0 (*schemas, ATHENA_SCHEMA) == 0) {
			window->priv->settings_athena = g_settings_new (ATHENA_SCHEMA);
			break;
		}
	}
	...
	if (window->priv->settings_athena)
		g_signal_connect (window->priv->settings_athena,
				  "changed::" ATHENA_CLICK_POLICY,
