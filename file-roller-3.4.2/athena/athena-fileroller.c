...
static void
extract_to_callback (AthenaMenuItem *item,
		     gpointer          user_data)
{
	GList            *files;
	AthenaFileInfo *file;
	...
	default_dir = athena_file_info_get_parent_uri (file);
...
static void
extract_here_callback (AthenaMenuItem *item,
		       gpointer          user_data)
{
	GList            *files, *scan;
	AthenaFileInfo *file;
	...
	dir = athena_file_info_get_parent_uri (file);
...
static void
add_callback (AthenaMenuItem *item,
	      gpointer          user_data)
{
	GList            *files, *scan;
	AthenaFileInfo *file;
...
static FileMimeInfo
get_file_mime_info (AthenaFileInfo *file)
{
	...
	for (i = 0; archive_mime_types[i].mime_type != NULL; i++)
		if (athena_file_info_is_mime_type (file, archive_mime_types[i].mime_type)) {
			char *mime_type;
			char *content_type_mime_file;
			char *content_type_mime_compare;

			mime_type = athena_file_info_get_mime_type (file);
...
static gboolean
unsupported_scheme (AthenaFileInfo *file)
{
	gboolean  result = FALSE;
	GFile    *location;
	char     *scheme;

	location = athena_file_info_get_location (file);
...
static GList *
athena_fr_get_file_items (AthenaMenuProvider *provider,
			    GtkWidget            *window,
			    GList                *files)
{
	GList    *items = NULL;
	...
	if (unsupported_scheme ((AthenaFileInfo *) files->data))
			return NULL;
	...
	for (scan = files; scan; scan = scan->next) {
		...
		if (can_write) {
			AthenaFileInfo *parent;

			parent = athena_file_info_get_parent_info (file);
 			can_write = athena_file_info_can_write (parent);
		}
	}
	...
	if (all_archives && can_write) {
		...
		item = athena_menu_item_new ("AthenaFr::extract_here",
					       _("Extract Here"),
		...
		item = athena_menu_item_new ("AthenaFr::extract_to",
					       _("Extract To..."),
	...
	if (! one_compressed_archive || one_derived_archive) {
		...
		item = athena_menu_item_new ("AthenaFr::add",
					       _("Compress..."),
...
static void
athena_fr_menu_provider_iface_init (AthenaMenuProviderIface *iface)
{
	iface->get_file_items = athena_fr_get_file_items;
}


static void
athena_fr_instance_init (AthenaFr *fr)
{
}


static void
athena_fr_class_init (AthenaFrClass *class)
{
...
void
athena_fr_register_type (GTypeModule *module)
{
	static const GTypeInfo info = {
		sizeof (AthenaFrClass),
		(GBaseInitFunc) NULL,
		(GBaseFinalizeFunc) NULL,
		(GClassInitFunc) athena_fr_class_init,
		NULL,
		NULL,
		sizeof (AthenaFr),
		0,
		(GInstanceInitFunc) athena_fr_instance_init,
	};

	static const GInterfaceInfo menu_provider_iface_info = {
		(GInterfaceInitFunc) athena_fr_menu_provider_iface_init,
		NULL,
		NULL
	};
